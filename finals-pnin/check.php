        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    echo "<tr>";
                    echo "<td>" . $row['id'] . "</td>";
                    echo "<td><a href='profile.php?student_id=" . $row['student_id'] . "'>" . $row['first_name'] . "</a></td>";
                    echo "<td>" . $row['last_name'] . "</td>";
                    echo "<td>" . $row['class'] . "</td>";
                    echo "<td>" . $row['student_id'] . "</td>";
                    echo "<td>";
                    echo "<input type='checkbox' name='attendance[]' value='" . $row['student_id'] . "'>";
                    echo "<input type='hidden' name='first_names[" . $row['student_id'] . "]' value='" . $row['first_name'] . "'>";
                    echo "<input type='hidden' name='last_names[" . $row['student_id'] . "]' value='" . $row['last_name'] . "'>";
                    echo "</td>";
                    echo "</tr>";
                }
            } else {
