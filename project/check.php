...
$sql = "SELECT id, student_number, first_name, last_name, class, student_id FROM std_dt";
                ...
                <tbody>
                    ...
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            echo "<td>";
                            echo "<input type='checkbox' name='attendance[]' value='" . htmlspecialchars($row['student_id']) . "'>";
                            echo "<input type='hidden' name='first_names[" . htmlspecialchars($row['student_id']) . "]' value='" . htmlspecialchars($row['first_name']) . "'>";
                            echo "<input type='hidden' name='last_names[" . htmlspecialchars($row['student_id']) . "]' value='" . htmlspecialchars($row['last_name']) . "'>";
